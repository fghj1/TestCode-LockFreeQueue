...
CLockFreeQueue<T>::~CLockFreeQueue()
{
	...
	// Each thread deletion waiting pointer list 解除
...
typename CLockFreeQueue<T>::HazardPointer* CLockFreeQueue<T>::AllocateHazardPointer( VOID )
{
	...
	pHazardP = new HazardPointer();  // ~CLockFreeQueue()の'Hazard pointer 解除'で解除
	if( NULL == pHazardP )
	{
		char szError[256] = {0,};
...
typename CLockFreeQueue<T>::vectDelWaitingPtrList* CLockFreeQueue<T>::GetEachThdDelWaitingPtrList( __in DWORD dwThreadID )
{
	...
	{
		pvtDWPList = new vectDelWaitingPtrList;  // ~CLockFreeQueue()の'Each thread deletion waiting pointer list 解除'で解除
		if( NULL == pvtDWPList )
		{
			char szError[256] = {0,};
...
VOID CLockFreeQueue<T>::UpdateEachThdDelWaitingPtrList( __in UnitNode* pUnusefulNode )
{
	...
	if( 100 < pvtDWPList->size() )  // TODO: 適切な限界値に変えろ。
